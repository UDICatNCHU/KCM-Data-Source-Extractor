...
Which is forked from [jwlin](https://travis-ci.org/jwlin/ptt-web-crawler)  
main.py是呼叫crawler.py進行格式轉換給KCM使用

### 執行方式
    python main.py -b 看板名稱 -i 起始索引 結束索引 (設為 -1 則自動計算最後一頁)
    python main.py -b 看板名稱 -a 文章ID

### 以下為 `jwlin` 的crawler.py原有的執行方式
